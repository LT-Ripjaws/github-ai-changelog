...
export function RepoCard({ repo, syncProgress, onSync, onDelete }: RepoCardProps) {
          ...
          <div className="mb-3">
            {displayTotal > 0 ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground mt-1">Discovering commits...</p>
              </>
            )}
          </div>
        )}
            ...
            {repo.status === "ready" && !isSyncing && (
              <>
                <Link href={`/dashboard/repos/${repo.id}/commits`}>
                  <Button variant="outline" size="sm">Commits</Button>
                </Link>
