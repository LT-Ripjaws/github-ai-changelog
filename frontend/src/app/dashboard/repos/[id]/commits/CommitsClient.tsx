...
import type { Commit, PaginatedResponse, SearchResult } from "@/lib/types";
...
export default function CommitsClient({ repoId, repoName, initialData }: CommitsClientProps) {
  ...
  const fetchCommits = (p: number, cat: string) => {
    setLoading(true);
    setError(null);
    getCommits(repoId, { page: p, limit: 20, ...(cat ? { category: cat } : {}) })
      .then((res) => { setCommits(res.data); setMeta(res.meta); })
      .catch((err) => setError(err.response?.data?.message || "Failed to fetch commits"))
      .finally(() => setLoading(false));
  };

  useEffect(() => {
    // Only fetch when page/category changes from user interaction.
    ...
    if (page !== 1 || category !== "") fetchCommits(page, category);
  }, [page, category]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2" role="group" aria-label="Filter by category">
        <Badge
          ...
          role="button" aria-pressed={!category}
        >All</Badge>
        {CATEGORIES.map((cat) => (
          <Badge
            key={cat} variant="outline"
            ...
            role="button" aria-pressed={category === cat}
          >{cat}</Badge>
        ))}
        {isSearchMode && (
          <Badge variant="outline" className="cursor-pointer badge-linear-neutral bg-violet-600/15 text-violet-400 border-violet-500/30"
            onClick={() => { setIsSearchMode(false); setSearchResults([]); }} role="button"
          >Clear search</Badge>
        )}
      </div>

      {/* Semantic search */}
      <SemanticSearchBar repoId={repoId} onSearch={handleSearch} results={searchResults} loading={searchLoading} error={searchError} />

      {loading && (<div className="space-y-3">{[...Array(5)].map((_, i) => (<SkeletonCommitRow key={i} />))}</div>)}
      ...
      {!loading && !error && (
        <>
          {isSearchMode ? (
            searchResults.length === 0 ? (<EmptySearch query="your search" />) : (
              <div className="space-y-2">
                {searchResults.map((result) => (
                  <Link key={result.id} href={`/dashboard/repos/${repoId}/commits/${result.sha}`} prefetch
                    className="block card-linear p-4 hover:border-brand-indigo/20 transition-all">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <code className="text-xs bg-surface-2 px-1.5 py-0.5 rounded font-mono text-text-tertiary">{result.sha.slice(0, 7)}</code>
                          {result.category && (<Badge variant="outline" className={`text-[10px] px-1.5 py-0 capitalize ${CATEGORY_COLORS[result.category] || ""}`}>{result.category}</Badge>)}
                          <span className="text-xs text-text-tertiary tabular-nums">{(result.similarity * 100).toFixed(1)}% match</span>
                        </div>
                        <p className="text-sm font-medium text-text-primary truncate">{truncate(result.message.split("\n")[0], 120)}</p>
                        {result.aiChangelog && (<p className="text-xs text-text-secondary mt-1 truncate">{truncate(result.aiChangelog, 150)}</p>)}
                      </div>
                      <div className="text-right text-xs text-text-tertiary whitespace-nowrap flex flex-col items-end gap-1 tabular-nums">
                        ...
                        <div className="flex gap-2"><span className="text-emerald-400">+{result.additions}</span><span className="text-red-400">-{result.deletions}</span><span>{result.filesChanged}f</span></div>
                      </div>
                    </div>
                  </Link>
                ))}
            ...
            commits.length === 0 ? (<EmptyCommits />) : (
              <div className="space-y-2">
                {commits.map((commit) => (
                  <Link key={commit.id} href={`/dashboard/repos/${repoId}/commits/${commit.sha}`} prefetch
                    className="block card-linear p-4 hover:border-brand-indigo/20 transition-all">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <code className="text-xs bg-surface-2 px-1.5 py-0.5 rounded font-mono text-text-tertiary">{commit.sha.slice(0, 7)}</code>
                          {commit.category && (<Badge variant="outline" className={`text-[10px] px-1.5 py-0 capitalize ${CATEGORY_COLORS[commit.category] || ""}`}>{commit.category}</Badge>)}
                          {commit.isMergeCommit && (<Badge variant="outline" className="text-[10px] px-1.5 py-0 badge-linear-neutral">merge</Badge>)}
                        </div>
                        <p className="text-sm font-medium text-text-primary truncate">{truncate(commit.message.split("\n")[0], 120)}</p>
                        {commit.aiChangelog && (<p className="text-xs text-text-secondary mt-1 truncate">{truncate(commit.aiChangelog, 150)}</p>)}
                      </div>
                      <div className="text-right text-xs text-text-tertiary whitespace-nowrap flex flex-col items-end gap-1 tabular-nums">
                        <span>{formatDate(commit.committedAt)}</span>
                        <div className="flex gap-2"><span className="text-emerald-400">+{commit.additions}</span><span className="text-red-400">-{commit.deletions}</span><span>{commit.filesChanged}f</span></div>
                      </div>
                    </div>
                  </Link>
                ))}
