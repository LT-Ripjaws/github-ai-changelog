...
export class RepoEntity {
  ...
  @ManyToOne(() => UserEntity, (u) => u.repos, { onDelete: 'CASCADE' })
  @JoinColumn({ name: 'user_id' }) user: UserEntity;
  @Column({ name: 'github_repo_id' }) githubRepoId: string;
  @Column({ name: 'full_name' }) fullName: string;
  @Column() name: string;
  @Column({ nullable: true }) description: string;
  @Column({ name: 'default_branch', default: 'main' }) defaultBranch: string;
  @Column({ name: 'is_private', default: false }) isPrivate: boolean;
  @Column({ name: 'stars_count', default: 0 }) starsCount: number;
  ...
  @Column({ name: 'total_commits_synced', default: 0 }) totalCommitsSynced: number;
